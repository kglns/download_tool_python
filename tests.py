...
class TestDownloaders(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sftp_uri = 'ftp://sftpuser:UbSftp^1@kaung.dev:/test/hello.txt'
    ...
    def test_sftp_download_get_creds(self):
        self.assertEqual(self.sftp._extract_credentials(self.sftp_uri[0]), ('user', 'pw'))

    def test_sftp_download_get_hostname(self):
        self.assertEqual(self.sftp._extract_hostname(self.sftp_uri[0]), 'example.com')
    
    def test_sftp_download_get_remote_path(self):
        self.assertEqual(self.sftp._extract_remote_path(self.sftp_uri[0]), '/file')
    
    def test_sftp_download_generate_filename(self):
        self.assertEqual(self.sftp.generate_filename(self.sftp_uri[0]), 'example.com--file')
